...
\33[1;37m---------------------------------
 ...
 \x1b[1;92mTool type  : FILE-CLONING\x1b[1;96m""")
...
def method():
    ...
    def start(user):
     try:
        ...
        sys.stdout.write(f'\r {R}[{W} REDOY {R}] {P}({Y}{loop}{W} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
              heads=ua()
              ...
              if 6==random.randint(1,300):
                 oku.append(acc)
                 print('\033[1;32m[REDOY-OK] \033[1;32m'+acc+' \033[1;32m|\033[1;32m '+pswd)
                 open('/sdcard/REDOY-Ok.txt','a').write(f'{acc}|{pswd}\n')
                 break
              else:
                   continue   
